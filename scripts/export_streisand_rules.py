#!/usr/bin/env python3
"""Export generated Shadowrocket lists into Streisand-compatible JSON files."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--write", action="store_true", help="write generated Streisand files to disk")
    parser.add_argument(
        "--offline",
        action="store_true",
...
def build_rules(domains: list[str], action: str, bucket: str) -> list[dict[str, str]]:
...
def build_bucket_payload(name: str, description: str, domains: list[str], action: str, bucket: str) -> dict[str, Any]:
    return {
        "name": name,
        "description": description,
        "rules": build_rules(domains, action=action, bucket=bucket),
    }
...
def build_outputs() -> dict[Path, str]:
    ...
    payloads: dict[Path, dict[str, Any]] = {
        STREISAND_DIR / "ru-direct.streisand.json": build_bucket_payload(
            name="RU Direct",
        ...
        STREISAND_DIR / "ru-blocked-core.streisand.json": build_bucket_payload(
        ...
        STREISAND_DIR / "foreign-services.streisand.json": build_bucket_payload(
...
def main() -> int:
    ...
    if args.write:
        ...
        if changed_paths:
            print(f"Updated {len(changed_paths)} Streisand file(s).", file=status_stream)
        else:

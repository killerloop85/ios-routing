...
- (опционально) `streisand/routing-profile-split-qr.json` — компактный split-профиль для QR и crash-prone импорта.
- (опционально) `streisand/routing-profile-full.streisand-uri.txt` — import-ready `streisand://` URI для full-профиля.
- (опционально) `streisand/routing-profile-split-qr.streisand-uri.txt` — compact import-ready `streisand://` URI для QR.

Практическая оговорка:

- тяжёлый `routing-profile-split.json` можно хранить как reference JSON для сопровождения policy и диффов;
...
3. **Compact QR split-profile**:

Допускается отдельный компактный профиль для QR/import-сценариев, который не обязан быть 1:1 копией полного split-профиля. Его задача — сохранить ключевую семантику:

- `local/private` → `direct`
- `domain:ru`, `domain:su`, `geoip:ru` → `direct`
- небольшой curated proxy-core (`geosite:*` + короткий список high-value `domain:*`) → `proxy`
- `final` → `proxy`

Такой профиль публикуется как:
...
- `scripts/export_streisand_uri.py`
...
- Параметры:
  ...
  - `--experimental-split` — дополнительно выпускать diagnostic/reference split-артефакты.

Для URI-экспорта:

- `scripts/export_streisand_uri.py` работает **после** генерации Streisand JSON-профилей;
- читает `streisand/routing-profile-*.json`;
- выпускает import-ready `streisand://...` URI в `.streisand-uri.txt` только для safe-профилей, пригодных для практического import-flow;
- использует обратимо декодируемую схему:
  - `binary plist`
  - `base64(plist)`
  - `import/route://...`
  - `base64(import/route://...)`
  - финальный префикс `streisand://`.
...
streisand:
	...
	python3 scripts/export_streisand_uri.py --write
...
Import-ready URI нужны как пользовательский артефакт поверх стабилизированных JSON-профилей, а не вместо них.
...
1. JSON-профили остаются источником читаемой и тестируемой семантики.
2. URI-файлы являются transport/export-слоем для импорта в клиента.
3. Smoke-check обязан подтверждать, что:
   - URI декодируется;
   - внутри валидный `route`-plist;
   - структура профиля согласована с JSON-оригиналом.
